...
use super::scoring::{PlaybackStatus, is_better_selection};
use super::*;
use anyhow::Error;
...
impl MockApplier {
    fn new() -> Arc<Self> {
        Arc::new(Self {
            ...
            cache_clean_calls: Mutex::new(Vec::new()),
        })
    }
    fn take(&self) -> Vec<ApplyCall> {
        std::mem::take(&mut self.calls.lock().unwrap())
    }
    fn take_timeline(&self) -> Vec<TimelineCall> {
        std::mem::take(&mut self.timeline_calls.lock().unwrap())
    }
    fn take_text(&self) -> Vec<TextCall> {
        std::mem::take(&mut self.text_calls.lock().unwrap())
    }
}
...
impl PlayerStateApplier for MockApplier {
    ...
    fn apply_status<'a>(
        &'a self,
        _device_id: ManagedDeviceId,
        _status: fsct::definitions::FsctStatus,
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), Error>> + Send + 'a>> {
    ...
    fn apply_timeline<'a>(
        &'a self,
        device_id: ManagedDeviceId,
        timeline: Option<fsct::definitions::TimelineInfo>,
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), Error>> + Send + 'a>> {
        Box::pin(async move {
            self.timeline_calls.lock().unwrap().push(TimelineCall {
                device: device_id,
    ...
    fn apply_text<'a>(
        &'a self,
        device_id: ManagedDeviceId,
        text_id: fsct::definitions::FsctTextMetadata,
        text: Option<&'a str>,
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), Error>> + Send + 'a>> {
...
fn make_ids(n: usize) -> Vec<ManagedDeviceId> {
    (0..n).map(|_| Uuid::new_v4()).collect()
}
fn pid(n: u32) -> ManagedPlayerId {
...
fn default_state_with_title(title: &str) -> PlayerState {
    ...
    s.texts
        .get_mut_text(fsct::definitions::FsctTextMetadata::CurrentTitle)
...
async fn one_player_zero_devices_state_update_no_apply() {
    ...
    let p1 = pid(1);
    let _ = ptx.send(PlayerEvent::Registered {
        player_id: p1,
        self_id: "p1".into(),
    });
    let s1 = default_state_with_title("S1");
    let _ = ptx.send(PlayerEvent::StateUpdated {
        player_id: p1,
        state: s1,
    });
...
async fn assign_before_connect_then_connect_then_update() {
    ...
    let _ = ptx.send(PlayerEvent::Assigned {
        player_id: p1,
        device_id: d,
    });
...
async fn one_player_multiple_devices_unassigned_then_assign() {
    ...
    assert!(
        calls
            .iter()
            .any(|c| c.device == d2 && c.state == PlayerState::default())
    );
...
async fn assigned_to_disconnected_counts_as_assigned_to_other_device() {
    ...
    assert!(
        calls
            .iter()
            .any(|c| c.device == d_unassigned && c.state == PlayerState::default())
    );
    let _ = handle.shutdown().await;
}
...
fn is_better_selection_order_independence_three_cases() {
    ...
    assert_eq!(
        winner,
        Some(c_non_playing_assigned_here),
        "idle assigned-here should beat last selected and playing unassigned in this triad"
    );
    ...
    for perm in permute_indices(items.len()) {
        ...
        assert_eq!(
            sorted, baseline_sorted,
            "Sorting should be stable regardless of input order for the 3-case scenario"
        );
    }
}
...
fn is_better_selection_order_independence_six_players_and_sort_stability() {
    let p_a_playing_assigned_here = PlayerSelectionParams {
        status: PlaybackStatus::Playing,
        assignment: Assignment::AssignedToThisDevice,
        is_last_selected: false,
        has_metadata: false,
    };
    let p_b_user_selected_idle = PlayerSelectionParams {
        status: PlaybackStatus::Paused,
        assignment: Assignment::Unassigned,
        is_last_selected: false,
        has_metadata: false,
    };
    let p_c_playing_unassigned = PlayerSelectionParams {
        status: PlaybackStatus::Playing,
        assignment: Assignment::Unassigned,
        is_last_selected: false,
        has_metadata: false,
    };
    let p_d_playing_assigned_other = PlayerSelectionParams {
        status: PlaybackStatus::Playing,
        assignment: Assignment::AssignedToOtherDevice,
        is_last_selected: false,
        has_metadata: false,
    };
    let p_e_idle_assigned_here = PlayerSelectionParams {
        status: PlaybackStatus::Stopped,
        assignment: Assignment::AssignedToThisDevice,
        is_last_selected: false,
        has_metadata: false,
    };
    let p_f_idle_unassigned_last = PlayerSelectionParams {
        status: PlaybackStatus::Stopped,
        assignment: Assignment::Unassigned,
        is_last_selected: true,
        has_metadata: false,
    ...
    assert_eq!(
        base_winner,
        Some(p_a_playing_assigned_here),
        "Expected the strongest candidate to win"
    );
    ...
    for perm in permute_indices(items.len()) {
        ...
        assert_eq!(
            sorted, baseline_sorted,
            "Sorting should be stable regardless of input order"
        );
    }
}
...
fn is_better_selection_tie_broken_by_last_selected() {
    ...
    let x1 = PlayerSelectionParams {
        status: PlaybackStatus::Stopped,
        assignment: Assignment::Unassigned,
        is_last_selected: false,
        has_metadata: false,
    };
    let x2 = PlayerSelectionParams {
        status: PlaybackStatus::Stopped,
        assignment: Assignment::Unassigned,
        is_last_selected: true,
        has_metadata: false,
    }; // should win
    let x3 = PlayerSelectionParams {
        status: PlaybackStatus::Stopped,
        assignment: Assignment::Unassigned,
        is_last_selected: false,
        has_metadata: false,
    };
    let x4 = PlayerSelectionParams {
        status: PlaybackStatus::Stopped,
        assignment: Assignment::Unassigned,
        is_last_selected: false,
        has_metadata: false,
    ...
    assert_eq!(
        winner,
        Some(x2),
        "The one flagged as last selected should be preferred among equals"
    );
}
...
fn is_better_selection_penalizes_assigned_to_other_device() {
    ...
    let playing_other = PlayerSelectionParams {
        status: PlaybackStatus::Playing,
        assignment: Assignment::AssignedToOtherDevice,
        is_last_selected: false,
        has_metadata: true,
    };
    let idle_unassigned = PlayerSelectionParams {
        status: PlaybackStatus::Stopped,
        assignment: Assignment::Unassigned,
        is_last_selected: false,
        has_metadata: true,
    ...
    assert_eq!(
        winner,
        Some(idle_unassigned),
        "Idle unassigned should be preferred over playing assigned to other device"
    );
}
...
fn is_better_selection_both_playing_assignment_order() {
    ...
    let playing_here = PlayerSelectionParams {
        status: PlaybackStatus::Playing,
        assignment: Assignment::AssignedToThisDevice,
        is_last_selected: false,
        has_metadata: false,
    };
    let playing_unassigned = PlayerSelectionParams {
        status: PlaybackStatus::Playing,
        assignment: Assignment::Unassigned,
        is_last_selected: false,
        has_metadata: false,
    ...
    for (items, expected) in cases {
        ...
        assert!(
            stable,
            "Winner should be order independent for pairwise playing comparison"
        );
...
fn is_better_selection_idle_assigned_here_beats_playing_unassigned_beats() {
    ...
    let idle_here = PlayerSelectionParams {
        status: PlaybackStatus::Stopped,
        assignment: Assignment::AssignedToThisDevice,
        is_last_selected: false,
        has_metadata: false,
    ...
    assert_eq!(
        winner,
        Some(idle_here),
        "Playing unassigned should beat idle assigned-here"
    );
}
...
fn is_better_selection_last_selected_breaks_tie_when_both_playing_same_assignment() {
    ...
    let a = PlayerSelectionParams {
        status: PlaybackStatus::Playing,
        assignment: Assignment::Unassigned,
        is_last_selected: false,
        has_metadata: false,
    };
    let b = PlayerSelectionParams {
        status: PlaybackStatus::Playing,
        assignment: Assignment::Unassigned,
        is_last_selected: true,
        has_metadata: false,
    ...
    assert_eq!(
        winner,
        Some(b),
        "Last selected should win among identical playing candidates"
    );
}
...
fn is_better_selection_four_players_permutation_and_sort() {
    ...
    let p1 = PlayerSelectionParams {
        status: PlaybackStatus::Playing,
        assignment: Assignment::AssignedToThisDevice,
        is_last_selected: false,
        has_metadata: false,
    };
    let p2 = PlayerSelectionParams {
        status: PlaybackStatus::Paused,
        assignment: Assignment::AssignedToThisDevice,
        is_last_selected: false,
        has_metadata: false,
    };
    let p3 = PlayerSelectionParams {
        status: PlaybackStatus::Playing,
        assignment: Assignment::Unassigned,
        is_last_selected: false,
        has_metadata: false,
    };
    let p4 = PlayerSelectionParams {
        status: PlaybackStatus::Stopped,
        assignment: Assignment::Unassigned,
        is_last_selected: false,
        has_metadata: false,
    ...
    for perm in permute_indices(items.len()) {
        ...
        assert_eq!(
            sorted, baseline_sorted,
            "Sorting should be stable for the 4-player nuanced set"
        );
    }
}
...
fn is_better_selection_all_assigned_to_other_device_picks_nothing() {
    ...
    let idle_other_1 = PlayerSelectionParams {
        status: PlaybackStatus::Paused,
        assignment: Assignment::AssignedToOtherDevice,
        is_last_selected: false,
        has_metadata: false,
    };
    let idle_other_2_last = PlayerSelectionParams {
        status: PlaybackStatus::Stopped,
        assignment: Assignment::AssignedToOtherDevice,
        is_last_selected: true,
        has_metadata: false,
    };
    let idle_other_3 = PlayerSelectionParams {
        status: PlaybackStatus::Stopped,
        assignment: Assignment::AssignedToOtherDevice,
        is_last_selected: false,
        has_metadata: false,
    ...
    assert_eq!(
        winner, None,
        "Among candidates all assigned to other devices, none should win"
    );
}
...
async fn timeline_update_triggers_partial_apply_only() {
    ...
    let _ = ptx.send(PlayerEvent::TimelineUpdated {
        player_id: p1,
        timeline: tl.clone(),
    });
    short_wait().await;
    ...
    assert!(
        full_calls.is_empty(),
        "Timeline update should not trigger full apply_to_device"
    );
...
async fn text_update_triggers_reassignment_and_full_apply() {
    ...
    let _ = ptx.send(PlayerEvent::TextMetadataUpdated {
        player_id: p2,
        metadata: FsctTextMetadata::CurrentTitle,
        text: Some(new_title.clone()),
    });
    short_wait().await;
    ...
    assert_eq!(
        full_calls.len(),
        1,
        "Text update on selected player should trigger full apply_to_device"
    );
    ...
    assert!(
        applier.take_text().is_empty(),
        "No partial text apply expected when full apply occurs after text update"
    );
...
async fn status_update_reassigns_and_full_apply() {
    ...
    let _ = ptx.send(PlayerEvent::StatusUpdated {
        player_id: p2,
    ...
    assert_eq!(
        calls.len(),
        1,
        "Expected one full apply after status change causing reassignment"
    );
    assert_eq!(calls[0].device, d);
...
fn scoring_order_test() {
    ...
    const PLAYER_SELECTION_PARAMS_ALL_COMBINATIONS: [PlayerSelectionParams; 36] = [
        ...
        PlayerSelectionParams {
            ...
            is_last_selected: true,
            has_metadata: true,
        }, // 10 -> 6 (last selected) + 4 (metadata) = 9
        /* 19 */
        PlayerSelectionParams {
            status: PlaybackStatus::Paused,
            assignment: Assignment::Unassigned,
            is_last_selected: true,
            has_metadata: true,
        }, // 13 -> 6 (last selected) + 4 (metadata) + 3 (paused) = 13
        ...
        PlayerSelectionParams {
            ...
            is_last_selected: true,
            has_metadata: false,
        }, // 22 -> 16 (playing) + 6 (last selected) = 22
        /* 23 */
        PlayerSelectionParams {
            status: PlaybackStatus::Playing,
            assignment: Assignment::Unassigned,
            is_last_selected: true,
            has_metadata: true,
        }, // 26 -> 16 (playing) + 6 (last selected) + 4 (metadata) = 26
    ...
    for (i, player_selection_params) in PLAYER_SELECTION_PARAMS_ALL_COMBINATIONS.iter().enumerate() {
        ...
        assert!(
            score > last_score || (score == 0 && last_score == 0),
            "Scores should be increasing, or zero in the begining"
        );
        if score <= last_score && score != 0 {
